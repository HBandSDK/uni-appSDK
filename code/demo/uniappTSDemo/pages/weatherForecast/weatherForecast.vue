<template>
	<view>
		...
		<button style="margin-top: 40rpx;" @click="readData">读取天气基本信息</button>
		<button style="margin-top: 40rpx;" @click="settingData">设置天气基本信息</button>
		<button style="margin-top: 40rpx;" @click="SendWeatherForecastDataManager">设置天气信息</button>
		<view class="box">
			<view> 天气开关：
				<switch :checked="weatherSwitch" @change="weather" />
			</view>
			<view> 摄氏度/华氏度：
				<switch :checked="unitSwitch" @change="unit" /> false 摄氏度 true 华氏度
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			SendWeatherForecastDataManager() {
				...
				let everydayData = this.everydayData;
				let dateDay = this.getDateday();

				dateDay.forEach((item, index) => {
				...
				veepooFeature.veepooSendWeatherForecastDataManager(value, function (e) {
					console.log("e=>", e)
				})
			},
			...
			weather() {
				let weatherSwitch = this.weatherSwitch;
				this.weatherSwitch = !weatherSwitch
			},

			unit() {
				let unitSwitch = this.unitSwitch;
			...
			notifyMonitorValueChange() {
				...
				veepooBle.veepooUniAppSDKNotifyMonitorValueChange(function (e) {
					...
					if (e.type == 10 && e.content) {
						...
						self.weatherSwitch = device.switch || e.switch;
						self.unitSwitch = device.unit == '摄氏度' ? false : true;
					}
				})
			},
		}
...
<style>
	/* pages/weatherForecast/index.wxss */
</style>