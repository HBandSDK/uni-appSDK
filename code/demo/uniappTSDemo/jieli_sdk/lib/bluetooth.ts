...
class ScanImpl implements IScan {
	...
	constructor() {
		...
		this._platform = uni.getSystemInfo().platform
	...
	private _stopScan() {
		...
		// App 端(app-plus)没有实现 uni.offBluetoothDeviceFound，直接调用抛 "is not a function"，
		// 会把同步调用方整条链掐死(OTA 回连的 connectDevice 就栽过)。小程序端有实现，正常调用。
		if (typeof (uni as any).offBluetoothDeviceFound === 'function') {
...
class ConnectImpl implements IConnect {
	...
	// 自己维护的建连超时。App 端 createBLEConnection 的 timeout 参数不可信(实测传了也不回调)，
	// 必须有一个 JS 侧完全自控的定时器，超时主动上报失败，否则上层只能干等大超时且无从重试。
	private _connectTimeoutMap = new Map<string, any>()
	private _platform = 'android'
	constructor() {
		setTagEnable(this._tag, true)
		this._platform = uni.getSystemInfoSync().platform;
	}
	/**
	 * 设置连接配置
	 */
	...
	public onBluetoothAdapterAvailable(available : boolean) {
		this._bluetoothAdapterAvailable = available
		if (!available) {
			// 同 offBluetoothDeviceFound：App 端没有该 API，需判空
			if (typeof (uni as any).offBLEConnectionStateChange === 'function') {
	...
	private _doCreateBLEConnection(device : BluetoothDevice) {
		...
		// 不依赖上面这个 timeout：App 端实测传了也不回调，真正的兜底是下面的
		// _startConnectTimeout(JS 侧自控定时器)。
		...
		connectOption.fail = (e) => {
			...
			if (e.errCode == -1) {
				...
				// 列表里，后续所有连接请求都被静默去重，回连只能等到超时。失败必须上报，
				// 让 DeviceManager 清掉 connecting 状态、上层收到 status=3 后重扫重试。
				this._onConnectFailed(device, e)
			}
		}
		// 安卓：远端断开时本类从不调 closeBLEConnection(见 _onConnectDisconnect)，底层
		// BluetoothGatt 不会被释放；对同一 MAC 再次 createBLEConnection 会复用残留 gatt
		// 走 autoConnect(后台慢连)，success/fail 都不回调——OTA 回连(设备重启)必踩。
		// 所以先 close 清残留。本方法只在系统确认"当前未连接"后进入，close 不会误伤活连接。
		// 注意：绝对不能把 create 放进 close 的回调里等它——实测对一个已断开的 deviceId 调
		// close，success/fail/complete 一个都不回调，create 就永远发不出去。这里发完 close
		// 就不管它，固定延时给底层释放 gatt 的时间，然后无条件 create。
		try {
			uni.closeBLEConnection({ deviceId: device.deviceId, success: () => { }, fail: () => { } })
		} catch (e) {
			loge('closeBLEConnection 异常(忽略)：' + JSON.stringify(e), this._tag)
		}
		this._startConnectTimeout(device)
		setTimeout(() => {
			console.log("========杰里开始连接=========");
			uni.createBLEConnection(connectOption)
		}, 300)
	}
	/** 启动 JS 侧建连超时：底层完全静默时也能把失败捅到上层，让回连有机会重试 */
	private _startConnectTimeout(device : BluetoothDevice) {
		this._clearConnectTimeout(device.deviceId)
		const ms = this._connectConfigure.timeout && this._connectConfigure.timeout > 0
			? this._connectConfigure.timeout : 15000
		const timer = setTimeout(() => {
			this._connectTimeoutMap.delete(device.deviceId)
			// 走到这里说明底层既没 success 也没 fail。此时设备还挂在 connecting 列表里，
			// 不上报失败的话后续连接请求会被静默去重掉。
			loge('建连超时(JS兜底 ' + ms + 'ms)，主动上报失败：' + device.deviceId, this._tag)
			this._onConnectFailed(device, {
				errCode: BluetoothErrorConstant.ERROR_CONNECTION_FAIL,
				errMsg: 'connect timeout'
			})
		}, ms)
		this._connectTimeoutMap.set(device.deviceId, timer)
	}
	private _clearConnectTimeout(deviceId : string) {
		const timer = this._connectTimeoutMap.get(deviceId)
		if (timer != undefined) {
			clearTimeout(timer)
			this._connectTimeoutMap.delete(deviceId)
		}
	}
	...
	private _negotiateMTUAndDiscover(device : BluetoothDevice) {
		if (this._platform == 'android' || this._platform == 'ohos') {
			...
			// iOS 上应用层既设不了 MTU(setBLEMTU 仅安卓 5.1+ 有效)，也读不到(uni-app 无 getBLEMTU，
			// 那是微信小程序独有的 API)，因此只能使用固定值。185 为 iOS 默认请求的 ATT MTU。
			const iosMTU = 185;
			console.log('[OTA诊断] iOS 固定 MTU=' + iosMTU + ' (应用层无法读取真实协商值)');
			logv('iOS MTU 使用固定值：' + iosMTU, this._tag);
			this._updateDeviceIdMtu(device.deviceId, iosMTU)
			this._onMTUChange(device, iosMTU)
			this._getBLEDeviceServices(device);
		}
	}
	/** 断开已连接设备 */
	...
	private _registerConnStatusListener() {
		...
		uni.onBLEConnectionStateChange(resFun);
	}
	/**
	 * 获取所有服务
	 */
	...
	private _notifyBLECharacteristicValueChange(obj : {
		...
		uni.notifyBLECharacteristicValueChange({
			...
			success: (res) => {
				...
				// 安卓 plus BLE 不串行化：使能通知的 CCCD 描述符写未完成就紧接着写认证包，
				// 会触发 GATT ERROR_GATT_WRITE_REQUEST_BUSY，认证包被静默丢弃、设备收不到 → 无回包 → 认证超时。
				// 延时让 CCCD 写落定后再继续（connectSuccess → 认证）。微信小程序内部已串行化，多等 500ms 无副作用。
				setTimeout(() => {
					callback.notifySuccess()
				}, 500)
			},
